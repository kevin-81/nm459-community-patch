...
doTileObservationLogic
    ...
    STA ObjectUpdateByte ;; reset ObjectUpdateByte
                         ...
                         ;; lets the update know there was a solid collision
...
doEraseBox_bank18:
    ;; draw box will update one metatile row at a time, so at max, it will be
    ;; updating 16x4 tiles (64) x 3 bytes per tile (192). This makes multple
    ;; frames of updates easy. If the queue flag bit 0 is activated, that means
    ;; that we have moved on to another row, but are not done yet.
     RTS
...
doDrawHud_bank18:
    ...
    ASL ;; multiplied by two, since metatiles are 16x16, but PPU addresses
        ;; are 8x8.
    STA temp
