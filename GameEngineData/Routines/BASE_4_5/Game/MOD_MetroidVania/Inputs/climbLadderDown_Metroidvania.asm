
;; Ladder script.

    ;; If up is engaged, check for a collision with a ladder. Check the pixel
    ;; just below feet, and the pixel just below top of bounding box.

    TXA
    PHA
    LDX player1_object

    ;; For checking bottom boundary.
    LDA Object_y_hi,x
    CLC
    ADC #$24 ;; height of player, bbox, wherever you want the bottom bounds to test
    ADC #$01 ;; ladder speed
    BCC +notAtBottom
        GetActionStep player1_object
        CMP #$03 ;; in this module, the player's action step 3 is for climbing
        BNE +notAtBottom

        LDA #BOUNDS_TOP ;#$02
        CLC
        ADC #$02
        STA newY

        LDA Object_x_hi,x
        STA newX

        LDA currentNametable
        CLC
        ADC #$10
        STA currentNametable
        STA Object_screen,x
        STA camScreen
        STA temp2

        WarpToScreen #$00, temp2, #$00
        JMP +notClimbing
    +notAtBottom:

    LDA Object_x_hi,x
    CLC
    ADC #$08 ;; MIDDLE OF PLAYER
    STA temp

    JSR getPointColTable

    LDA Object_y_hi,x
    CLC
    ADC #$11 ;; height of player plus arbitrary number to check.
             ;; (if player is #$10 pixels tall, maybe put #$12 here to check
             ;; just below his feet).
    STA temp1

    ;; Check for ladder tile, which is tile type 10 ($0A) in this module.
    CheckCollisionPoint temp, temp1, #$0A, tempA ;; is it a ladder?
    BNE +notLadder

    ;; There is a ladder under my feet..
    LDA Object_y_hi,x
    CLC
    ADC #$01 ;; ladder speed
    STA Object_y_hi,x

    GetActionStep player1_object
    CMP #$03 ;; in this module, the player's action step 3 is for climbing
    BEQ +alreadyOnLadder
        LDA gamepad
        AND #%11000000
        BNE +alreadyOnLadder ;; will turn off ladder if right or left is pressed.

        ChangeActionStep player1_object, #$03

        LDA Object_direction,x
        AND #%00001111
        STA Object_direction,x
    +alreadyOnLadder:
    JMP +notClimbing

+notLadder:
   GetActionStep player1_object
   CMP #$03
   BNE +notClimbing
       ChangeActionStep player1_object, #$00
    +notClimbing

    PLA
    TAX
    RTS

