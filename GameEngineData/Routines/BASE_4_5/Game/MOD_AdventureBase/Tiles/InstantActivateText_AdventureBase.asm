    ...
    ;; and then will change it's collision type to walkable, so that the
    ;; collision only happens one time per screen load.

    ;; From the jump, right now the tile offset should be loaded in temp1 and
    ;; temp2 should determine what collision table we should look in.
