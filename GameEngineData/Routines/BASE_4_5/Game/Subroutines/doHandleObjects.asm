...
doHandleObjects:
    ...
    LDA gameStatusByte
    AND #%00000001 ;;; this will skip object handling.
    BEQ +skipResetNpcTriggerIfObjectIsOff
        LDA npcTrigger
        AND #%11111110
        STA npcTrigger
    +skipResetNpcTriggerIfObjectIsOff
    ...
    loop_doHandleObjectsLoop:
        ...
        ;;  2) We have the states from each button (8 buttons, two controllers)
        ;;     from input management. So now we compare it to this object, and
        ;;     whether or not it should have any behaviors associated with it's
        ;;     pressed, down, released, or up states.
        ;;  3) Switch banks to the one that contains our LUTs for object data
        ;;     (sizes and whatnot)
        ...
        LDA Object_type,x
        STA tempObjType ;; not corrupted by any other routines, used in timer
                        ;; handlings so no reference to bank1c is needed.
        ...
        SwitchBank #$18
            ...
            ;; now we update the object.  This includes pushing temp
            ;; positions to new positions, handling timers, doing AI mode
            ;; updates and whatnot, etc, and anything that should happen
        ...
        SwitchBank #$1C
            ...
            BEQ ObjectDoesNotDraw
                ...
                ;; Additional object specific drawing stuff would go here.
