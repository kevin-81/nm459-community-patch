...
currentlyUpdatingQueuedTiles:
    ...
    doDrawBoxLoop_inner:
        ...
        INC tempx ;; temp x is the TILE offset, where as y /
                  ;; scrollOffsetCounter represents the ram offset, which
                  ;; consists of high byte, low byte, AND tile.
    ...
    LDA queueFlags
    ASL  ;; this shifts these bits to the left two places. 0001 will become
    ASL  ;; 0100, meaning that we are driving all attributes to box
         ;; attributes. 0011 will become 1100, meaning we are restoring the
         ;; screen's attributes.
    AND #%00001100 ;; this will scrape off the bits if we have passed
                   ;; attribute queue.
    STA queueFlags
...
currentlyUpdatingQueuedAttribtues:
    ;; the max amount of attributes to be updated on a screen is 64. There are
    ;; three bytes needed to be pushed into the scrollUpdateRam to update in the
    ;; next refresh. All attributes, then, can be updated within 256 bytes, so
    ;; there is no reason to go row by row as we do with metatiles, which need
    ;; 4 tile definitions x hi and low bytes for each.
